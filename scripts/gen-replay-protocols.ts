...
import { displayPath, writeJson, writeText } from "./lib/fs.ts";
...
async function main(): Promise<void> {
  ...
  for (const [hash, body] of tables) {
    await writeJson(path.join(outDir, `${hash}.json`), { protocol_builds: sharedBy.get(hash), ...JSON.parse(body) });
  }

  const buildNumbers = Object.keys(builds).map(Number).sort((a, b) => a - b);
  await writeJson(path.join(outDir, "index.json"), { builds, latest: buildNumbers[buildNumbers.length - 1] });
  ...
  console.log(
    `gen-replay-protocols: ${files.length} builds, ${tables.size} distinct tables -> ${displayPath(outDir)}`
  );
}
